...
class DiffSummary:
    ...
    def __repr__(self):
        if self.num_changed_lines == 0:
            return "DiffSummary:: no changes"
        elif self.num_changed_lines == 1:
            return "DiffSummary:: 1 changed line"
        elif self.num_changes == 1:
            return "DiffSummary: %d lines changed in 1 section" % (self.num_changed_lines, )
        return "DiffSummary: %d lines changed in %d sections" % (self.num_changed_lines, self.num_changes)
...
<head>
    <title>Diff</title>
    <link rel="stylesheet" type="text/css" href="reviewboard/htdocs/media/rb/css/diffviewer.css" />
</head>
<body>
    %s
</body>
...
"""
    ...
    def toHTML(self):
        ...
        text = render_to_string('diffviewer/diff_file_fragment.html',
                                { 'file': self,
                                  'collapseall': True })
        return self.html_template % (text, )
